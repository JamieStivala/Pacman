...
import game.items.stationery.Coin;
import game.items.stationery.Empty;
import game.items.stationery.Wall;
import game.map.generator.Seed;
...
/**
 * This class extends the seed class. Here the building and rendering of the game.map is done
 */
public class PacmanMap extends Seed {
    ...
    * This is called the represent the spawnBox all the ghosts are going to be inside of
     ...
     *
     * This was done to fix a bug where the game would go at one side of the game.map, teleport to the other side and be stuck in the wall.
     */
    ...
    /**
     * Draws the game.map with all the textures (Coin, Wall, Empty) and adds it to the 2D array called game textures.
    ...
    /**
     * This will paint the game.map in a BufferedMap beforehand so it doesn't render on request causing lag.
     * The game.map also requires this so when coins are picked everything is rendered again
     */
    public void paint() {
    ...
    /**
     * This is a getter for the pre-rendered buffered game.map
