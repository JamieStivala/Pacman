...
import pacman.items.stationery.Coin;
import pacman.items.stationery.Empty;
import pacman.items.stationery.Wall;
import pacman.map.generator.Seed;
...
/**
 * This class extends the seed class. Here the building and rendering of the pacman.map is done
 */
public class PacmanMap extends Seed {
    ...
    * This is called the represent the spawnBox all the ghosts are going to be inside of
     ...
     *
     * This was done to fix a bug where the pacman would go at one side of the pacman.map, teleport to the other side and be stuck in the wall.
     */
    ...
    /**
     * Draws the pacman.map with all the textures (Coin, Wall, Empty) and adds it to the 2D array called game textures.
    ...
    /**
     * This will paint the pacman.map in a BufferedMap beforehand so it doesn't render on request causing lag.
     * The pacman.map also requires this so when coins are picked everything is rendered again
     */
    public void paint() {
    ...
    /**
     * This is a getter for the pre-rendered buffered pacman.map
